...
export const QR_EXPORT_BACKGROUND = "#F8FBF8";
...
function renderSvg(grid: StyledQrGrid, layout: QrExportLayout): string {
  const { canvasSize } = layout;
  const finderLayout = finderDrawLayout(layout);

  const rects = grid.modules
    .filter((mod) => mod.isDark && !isFinderModule(mod.row, mod.col, grid.size))
    .map((mod) => {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${canvasSize}" height="${canvasSize}" viewBox="0 0 ${canvasSize} ${canvasSize}" shape-rendering="crispEdges"><rect width="${canvasSize}" height="${canvasSize}" fill="${QR_EXPORT_BACKGROUND}"/>${rects}${finderCircles}</svg>`;
}
...
function renderCanvas(
  ...
  const canvas = renderQrCaptureCanvas(grid, {
    ...
    shouldDrawModule: (mod) =>
      mod.isDark && !isFinderModule(mod.row, mod.col, grid.size),
  });
...
export async function downloadStyledQrGrid(
  ...
  const layout = computeQrExportLayout(grid.size, options.size);
  const filename = exportFilename(grid, layout, options.format);

  if (options.format === "svg") {
    triggerDownload(
      new Blob([renderSvg(grid, layout)], { type: "image/svg+xml" }),
      filename,
    );
    return;
  }

  const canvas = renderCanvas(grid, layout);
