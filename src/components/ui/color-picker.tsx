...
function resolveFloatingBounds(triggerEl: HTMLElement | null) {
  const pad = FLOATING_PADDING;
  let left = pad;
  let top = pad;
  let right = window.innerWidth - pad;
  let bottom = window.innerHeight - pad;

  const boundary = triggerEl?.closest("[data-playground-panel]");
  if (boundary) {
    const b = boundary.getBoundingClientRect();
    left = Math.max(left, b.left + pad);
    top = Math.max(top, b.top + pad);
    right = Math.min(right, b.right - pad);
    bottom = Math.min(bottom, b.bottom - pad);
  }
...
function computeFloatingPosition(
  ...
  const bounds = resolveFloatingBounds(triggerEl);
  ...
  top = Math.max(bounds.top, Math.min(top, bounds.bottom - panel.height));

  let left = trigger.left;
  if (left + panel.width > bounds.right) {
  ...
  if (left + panel.width > bounds.right) {
    left = bounds.right - panel.width;
  }

  const originX = Math.max(
    8,
    Math.min(panel.width - 8, trigger.left + trigger.width / 2 - left),
  );
  const originY = placement === "bottom" ? 0 : panel.height;
...
function checkerboardImage(): string {
  return "repeating-conic-gradient(var(--checker-b) 0 25%, var(--checker-a) 0 50%, var(--checker-b) 0 75%, var(--checker-a) 0 100%)";
}
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: focused ? "0 0 0 2px var(--brand)" : undefined,
      }}
    >
      <div
        className={cn(
...
function AlphaSlider({
  ...
  return (
    <Slider
      ...
      thumbBorderColor="rgba(255,255,255,0.9)"
      trackStyle={{
        backgroundImage: `linear-gradient(to right, ${transparentColor} 0%, ${solidColor} 100%), ${checkerboardImage()}`,
        backgroundSize: `100% 100%, ${CHECKER_TILE}px ${CHECKER_TILE}px`,
  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        className={cn(
          "flex items-center justify-between gap-2 h-9 px-3 text-[13px] bg-transparent hover:bg-hover hover:text-foreground transition-colors duration-80 outline-none focus-visible:ring-1 focus-visible:ring-brand cursor-pointer",
  ...
  (
    ...
    return (
      <div
        ...
        className={cn(
          "flex items-center h-9 px-2 bg-transparent hover:bg-hover active:bg-active transition-colors duration-80 focus-within:ring-1 focus-within:ring-brand select-none",
          shape.input,
...
function EyeDropperButton({ onPick }: { onPick: (hex: string) => void }) {
  ...
  return (
    <button
      ...
      className={cn(
        "flex items-center justify-center h-9 px-3 text-muted-foreground bg-transparent hover:bg-hover hover:text-foreground active:bg-active transition-colors duration-80 outline-none focus-visible:ring-1 focus-visible:ring-brand cursor-pointer",
        shape.input
      )}
    >
...
const ColorPickerPopover = forwardRef<HTMLDivElement, ColorPickerPopoverProps>(
  (
    ...
    const updateFloatingPosition = useCallback(() => {
      ...
      const panelSize = {
        width: panelNode?.offsetWidth ?? PANEL_WIDTH,
        height: panelNode?.offsetHeight ?? PANEL_ESTIMATED_HEIGHT,
      };

      setFloatingPos(
        computeFloatingPosition(triggerRect, panelSize, trigger),
      );
    }, []);
    ...
    }, [open, panelEl, updateFloatingPosition]);

    const resolvedPos = useMemo(() => {
      if (!open) return null;
      if (floatingPos) return floatingPos;
      const trigger = triggerRef.current;
      if (!trigger) return null;
      return computeFloatingPosition(
        trigger.getBoundingClientRect(),
        { width: PANEL_WIDTH, height: PANEL_ESTIMATED_HEIGHT },
        trigger,
      );
    }, [open, floatingPos]);
    ...
    return (
      <div ref={ref} className="inline-flex">
        <button
          ...
          aria-expanded={open}
          className={cn(
            "flex items-center gap-2 h-9 px-2 border border-border bg-transparent hover:bg-hover transition-colors duration-80 outline-none focus-visible:ring-1 focus-visible:ring-brand cursor-pointer",
            shape.input,
            triggerClassName
          )}
          style={{ fontWeight: fontWeights.medium }}
        ...
        {open && resolvedPos && typeof document !== "undefined" && createPortal(
          <div
            style={{
              position: "fixed",
              top: resolvedPos.top,
