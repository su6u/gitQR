...
/** Cap step-dot count so wide ranges (e.g. 20–56%) don't become a solid dotted line */
const MAX_STEP_DOTS = 12;
...
function buildStepDots(
  ...
  const range = max - min;
  if (range <= 0) return [{ value: min, percent: 0 }];

  const rawCount = Math.round(range / step) + 1;
  let dotStep = step;
  if (rawCount > MAX_STEP_DOTS) {
    dotStep =
      Math.max(
        step,
        Math.ceil(range / (MAX_STEP_DOTS - 1) / step) * step,
      );
  }

  const dotCount = Math.round(range / dotStep) + 1;
  return Array.from({ length: dotCount }, (_, i) => {
    const v = Math.min(max, min + i * dotStep);
    return { value: v, percent: (v - min) / range };
  });
}
...
const Slider = forwardRef<HTMLDivElement, SliderProps>(
  (
    ...
    const stepDots = useMemo(
      () => (showSteps ? buildStepDots(min, max, step) : []),
                ...
                {stepDots.map(({ value: v, percent }) => (
                  ...
                  >
                    <motion.div
                      ...
                      style={{
                        backgroundColor:
                          "var(--slider-step-dot, var(--muted-foreground))",
                        opacity: "var(--slider-step-dot-opacity, 0.3)",
                      }}
                    />
                  </div>
                ))}
  ...
  (
    ...
    return (
      ...
      <motion.div
        ...
        initial={false}
        animate={{
          outline: isFocused ? "1px solid var(--brand)" : "1px solid transparent",
        }}
        transition={spring.fast}
        onPointerDown={handlePointerDown}
        onPointerMove={handlePointerMove}
